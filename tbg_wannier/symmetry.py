...
from .utils import rot2, sx, sy, sz, I2, kron, make_U_from_wanniers, make_wanniers_from_U
...
class SymmetryOperator:
    ...
    def sewing_matrices(
        ...
        for ik in range(Nk):
            ...
            if not self.is_antiunitary:
                S[ik] = eigvecs[ikp].conj().T @ Remb.T @ D_full @ eigvecs[ik]  # (Nk, nb, nb)
            else:
                S[ik] = eigvecs[ikp].conj().T @ Remb.T @ D_full @ eigvecs[ik].conj()  # (Nk, nb, nb)
        return S
...
class SymmetryGroup:
    ...
    def _compose_rep_on_mesh(self, rep_gen: np.ndarray,
                              rep_parent: np.ndarray, k_map_parent: np.ndarray,
                                gen_antiunitary_flag: bool=False) -> np.ndarray:
        ...
        for ik in range(Nk):
            if gen_antiunitary_flag:
                out[ik] = rep_gen[int(k_map_parent[ik])] @ rep_parent[ik].conj()
            else:
    ...
    def build_D_band_from_group(
        ...
        *,
        ...
        if norb_internal is None:
            ...
            any_op = next(iter(self.generators.values()))
        ...
        reps[self.identity] = np.tile(np.eye(nb, dtype=complex)[None, :, :], (Nk, 1, 1))

        # sewing for generators (direct)
        for gname in generator_names:
            reps[gname] = self.generators[gname].sewing_matrices(
                lat=lat, eigvecs=eigvecs)

        # k-maps for all elements needed for composition
        k_maps = self.k_maps_all(k_frac, Nmesh)
        ...
        for name in elem_names:
            ...
            if gen not in reps:
                ...
                raise KeyError(f"Missing sewing matrix for generator '{gen}' needed to build element '{name}'")

            # Pass flag indicating whether the generator is antiunitary
            gen_anti = bool(self.generators[gen].is_antiunitary)
            reps[name] = self._compose_rep_on_mesh(reps[gen], reps[parent], k_maps[parent], gen_antiunitary_flag=gen_anti)
    ...
    def build_D_wann_generators_from_EBRs(
        ...
        generators_needed = list(self.generators.keys())
        Nk = k_mesh.shape[0]
        # determine nwann
    ...
    def build_D_wann_from_group(
        ...
        *,
        ...
        D_wann_generators = self.build_D_wann_generators_from_EBRs(ebr_sequence, lat)
        # infer nwann
        ...
        for gname in self.generators.keys():
    ...
    def build_dmn_maps_irreducible(
        ...
        *,
        ...
        for isym, name in enumerate(elem_names):
            all_k_maps[isym] = self.k_map_for_element(name, k_frac, Nmesh)
        sym_kpt_map_full = all_k_maps.T.astype(int) + 1
        if not irreducible:
            # If not irreducible, return identity mappings
            full_to_irr = np.arange(1, Nk + 1, dtype=int)
            irr_kpts = np.arange(1, Nk + 1, dtype=int)
            return full_to_irr, irr_kpts, sym_kpt_map_full, elem_names
...
def symmetrize_with_C2zT(
        U: np.ndarray,
        eigvecs: np.ndarray,
        lat: MoireLattice,
        recipe: WannierizationRecipe,
):
    wanniers = make_wanniers_from_U(U, eigvecs)
    C2zT_op = group_TR.elements["C2zT*E"].op
    D_C2zT = C2zT_op.rep_matrix(lat).toarray()
    D_wanns, elem_names = group_TR.build_D_wann_from_group(
        lat, recipe.ebr_sequence, irr_kpts_indices=None
    )
    D_wanns_C2zT = D_wanns[elem_names.index("C2zT*E")]
    wan_tr = wanniers + np.einsum("ij,kjl->kil", D_C2zT, wanniers.conj(), optimize=True) @ D_wanns_C2zT.transpose(0,2,1).conj()
    wan_tr = wan_tr / np.linalg.norm(wan_tr, axis=1, keepdims=True)
    U_sym = make_U_from_wanniers(wan_tr, eigvecs)
...
def symmetrize_u_matrix(
    ...
    *,
    ...
    """Symmetrize U(k) following the symWannier algorithm.
    
    This implementation follows the approach in:
    https://github.com/wannier-utils-dev/symWannier/blob/main/src/symwannier/amn.py
    
    The algorithm:
    1. Extract U at irreducible k-points
    2. For each irreducible k, average over its little group (star of k):
       The little group G_k consists of symmetries g such that g*k = k (up to reciprocal lattice vector).
       U_sym(k_irr) = (1/|G_k|) sum_{g in G_k} D_band(g,k_irr)† @ U(k_irr) @ D_wann(g,k_irr)
       Note: Uses U(k_irr), not U(gk) - the symmetry transformation is encoded in D_wann!
    3. Expand back to full mesh:
       For each full k, find the unique g such that g * k_irr = k (where k_irr is the irreducible rep)
       U(k_full) = U_sym(k_irr) @ D_wann(g, k_irr)
    ...
    group : SymmetryGroup
        The symmetry group.
    recipe : WannierizationRecipe
        Contains ebr_sequence for building D_wann.
    eigvecs : (Nk, dim, nb)
        Eigenvectors for building D_band.
    lat : MoireLattice
        The lattice.
    n_iter : int
        Number of symmetrization iterations.
    enforce_semiunitary : bool
        If True, project back to semi-unitary after each iteration.
    """
    U = np.asarray(U, complex)
    if U.ndim != 3:
    ...
    # Build irreducible k-point mappings
    full_to_irr, irr_kpts, sym_kpt_map, _ = group.build_dmn_maps_irreducible(
        lat, elem_names=elem_names, irreducible=True
    )
    
    # Convert to 0-based indices
    irr_kpts_indices = irr_kpts - 1  # (Nirr,)
    full_to_irr_0based = full_to_irr - 1  # (Nk,)
    sym_kpt_map_0based = sym_kpt_map - 1  # (Nirr, Ng)
    Nirr = len(irr_kpts_indices)
    # Build D_wann and D_band at irreducible k-points
    D_wann_irr, _ = group.build_D_wann_from_group(
        lat, recipe.ebr_sequence, irr_kpts_indices=irr_kpts_indices
    )  # (Ng, Nirr, nw, nw)
    D_band_irr, _ = group.build_D_band_from_group(
        lat, eigvecs, irr_kpts_indices=irr_kpts_indices
    )  # (Ng, Nirr, nb, nb)
    
    # Identify antiunitary elements
    is_anti_arr = np.array([
        bool(getattr(group.elements[name].op, "is_antiunitary", False)) 
        for name in elem_names
    ])
    
    # Build little group (star) for each irreducible k-point:
    # A symmetry g is in the little group of k if g*k = k (up to reciprocal lattice vector G)
    # In the discrete mesh, this means k_map[ik] == ik
    # little_group_mask[i_irr, ig] = True if symmetry ig leaves k_irr invariant
    little_group_mask = np.zeros((Nirr, Ng), dtype=bool)
    for i_irr in range(Nirr):
        ik_irr_full = int(irr_kpts_indices[i_irr])
        for ig in range(Ng):
            # Check if this symmetry maps k_irr to itself on the mesh
            ik_image = int(sym_kpt_map_0based[i_irr, ig])
            if ik_image == ik_irr_full:
                little_group_mask[i_irr, ig] = True
    little_group_sizes = little_group_mask.sum(axis=1)  # (Nirr,)
    U_cur = U.copy()
    # Step 1: Extract U at irreducible k-points
    U_irr = U_cur[irr_kpts_indices]  # (Nirr, nb, nw)    
    for _it in range(max(1, int(n_iter))):        
        # Step 2: Symmetrize at irreducible k-points (average over little group)
        U_irr_sym = np.zeros_like(U_irr, dtype=complex)
        
        for i_irr in range(Nirr):
            acc_irr = np.zeros((nb, nw), dtype=complex)
            if little_group_sizes[i_irr] == 1:
                # Only identity in little group, so just copy
                U_irr_sym[i_irr] = U_irr[i_irr]
                continue
            for ig in range(Ng):
                if not little_group_mask[i_irr, ig]:
                    continue
                # Unitary: acc += D_band†(g,k_irr) @ U(k_irr) @ D_wann(g,k_irr)
                tmp = D_band_irr[ig, i_irr].conj().T @ U_irr[i_irr] @ D_wann_irr[ig, i_irr]
                # Antiunitary: acc += D_band†(g,k_irr) @ conj(U(k_irr)) @ conj(D_wann(g,k_irr).T)
                if is_anti_arr[ig]:
                    tmp = tmp.conj()
                acc_irr += tmp
            U_irr_sym[i_irr] = acc_irr / float(little_group_sizes[i_irr])
        iden_irr = np.tile(np.eye(nb, nw, dtype=complex)[None, :, :], (Nirr, 1, 1))
        diff = iden_irr - U_irr_sym @ U_irr_sym.conj().transpose(0,2,1)
        print(f"  symmetrize iteration {_it+1}/{n_iter}: symmetrized irreducible k-points: diff = {np.sum(np.abs(diff)) / Nirr:15.5e}")
        if enforce_semiunitary:
        # Project each k-point back to semi-unitary
            for ik in range(Nirr):
                U_irr[ik] = scipy.linalg.polar(U_irr_sym[ik])[0]

    # Step 3: Expand back to full mesh
    U_new = np.zeros_like(U_cur, dtype=complex)
    print(f"  symmetrize {n_iter} times, expanding to full k-mesh")
    
    for ik in range(Nk):
        i_irr = int(full_to_irr_0based[ik])
        if ik == i_irr:
            # This k-point is itself an irreducible representative, so just copy the symmetrized version
            U_new[ik] = U_irr[i_irr]
            continue
        # Find which symmetry maps irr k-point to this full k-point
        # We need to find ig such that sym[ig] * k_irr = k (up to the mesh)
        ik_irr_full = int(irr_kpts_indices[i_irr])
        
        # Find the symmetry: check which ig maps ik_irr_full to ik
        ig_map = -1
        for ig in range(Ng):
            if int(sym_kpt_map_0based[i_irr, ig]) == ik:
                ig_map = ig
                break
        
        if ig_map == -1:
            # This shouldn't happen if mappings are consistent
            # Fall back to identity
            raise ValueError(f"No symmetry found mapping irreducible k-point {i_irr} to full k-point {ik}. Check sym_kpt_map consistency.")
        else:
            # Unitary: U(k_full) = U_sym(k_irr) @ D_wann(g, k_irr)
            # print(f"    Mapping irreducible k-point {i_irr} to full k-point {ik} using symmetry {elem_names[ig_map]} (unitary={not is_anti_arr[ig_map]})")
            U_new[ik] = D_band_irr[ig_map, i_irr] @ U_irr[i_irr] @ D_wann_irr[ig_map, i_irr].T.conj()
            if is_anti_arr[ig_map]:
                # Antiunitary: U(k_full) = conj(U_sym(k_irr)) @ conj(D_wann(g, k_irr).T)
                U_new[ik] = U_new[ik].conj()
    
    print('symmetrization complete.')
        # if do_C2zT:
        #     U_cur = symmetrize_with_C2zT(U_cur, eigvecs, lat, recipe)
        # Compute difference for monitoring
    # diff = np.sum(np.abs(U_cur - U)) / Nk
    return U_new